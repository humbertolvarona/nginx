...
if [ -n "$PHP_MEMORY_LIMIT" ]; then
    sed -i "s|^memory_limit = .*|memory_limit = ${PHP_MEMORY_LIMIT}|" /etc/php83/php.ini || true
    sed -i "/^php_admin_value\[memory_limit\]/d" /etc/php83/php-fpm.d/www.conf
    echo "php_admin_value[memory_limit] = ${PHP_MEMORY_LIMIT}" >> /etc/php83/php-fpm.d/www.conf
    success "PHP memory limit set to $PHP_MEMORY_LIMIT"
fi
...
if [ -f "$WEBROOT/index.php" ] || [ -f "$WEBROOT/index.html" ] || [ -f "$WEBROOT/index.htm" ]; then
    success "Index file found in $WEBROOT."
else
    warn "No index.php/html/htm found in $WEBROOT!"
fi
